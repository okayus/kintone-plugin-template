...
graph TB
    subgraph "入力（開発者が定義）"
        ...
        Widgets["カスタムウィジェット<br/>独自の入力コンポーネント"]
...
graph LR
    subgraph "RJSFからの入力"
        Props["props<br/>・value: 現在値<br/>・onChange: 値変更関数<br/>・formContext: 追加データ<br/>・idSchema: ID情報<br/>・formData: 全体データ"]
    end
...
3. **カスタムウィジェット**: 複雑なUIや動的な動作が必要な場合
4. **formContext**: 親子間のデータ共有と連携

この実装により、メンテナンス性が高く、拡張しやすいフォームが実現できます。